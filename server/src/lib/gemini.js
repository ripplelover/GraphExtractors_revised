...
export async function callGeminiWithImage(b64Image, prompt, mime = 'image/png') {
  ...
  const envList = (process.env.GEMINI_FALLBACK_MODELS || '').split(',').map(s => s.trim()).filter(Boolean);
  const candidates = envList.length ? envList : [
    'gemini-2.5-pro',
    'gemini-2.5-flash',
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      if (res.status === 429 || res.status === 503) {
        if (attempt < maxAttempts - 1) {
          const base = Math.pow(2, attempt) * 250; // 250ms, 500ms, 1000ms, 2000ms
          const jitter = Math.floor(Math.random() * 150);
          const retryAfterMs = lastRetryAfter ? Number(lastRetryAfter) * 1000 : 0;
          await sleep(Math.max(base + jitter, retryAfterMs));
          continue;
        }
        break; // exhausted retries for this model
      }
      // For 404, try next candidate; for other errors, stop early
      if (res.status !== 404) break;
      else break; // 404 -> try next model
    }
  }
  const err = new Error(`Gemini API error: ${lastErrText}`);
  if (lastStatus) err.statusCode = lastStatus;
  if (lastRetryAfter) err.retryAfter = lastRetryAfter;
  throw err;
}
