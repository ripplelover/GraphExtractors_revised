...
import { findDataValues, findColorEncoding, updateDataValues, getEditableColorRange, setColorRange, getDomainRangePair, setDomainRange, getActiveColors, setActiveColors } from "./utils/dataUtils";
                 ...
                 <div style={{ 
                  ...
                  <div style={{ display:'grid', gap:8 }}>
                    ...
                    <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                      {(() => {
                        const active = getActiveColors(spec, palette);
                        if (!active.length) return null;
                        return active.map((c:string, i:number)=> (
                          <input key={i} type="color" value={c} onChange={e=>{
                            const updated = active.slice();
                            updated[i] = e.target.value;
                            const next = setActiveColors(spec, updated);
                            setPalette(updated);
                      ...
                      <button className="btn" onClick={()=>{
                        const active = getActiveColors(spec, palette);
                        const nextRange = [...active, '#4e79a7'];
                        const next = setActiveColors(spec, nextRange);
                        setPalette(nextRange);
                        setSpec(next); saveProject();
                      }}>색상 추가</button>
                    </div>
                    {originalImageSrc && (
