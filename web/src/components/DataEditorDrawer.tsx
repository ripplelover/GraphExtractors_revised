...
import { findColorEncoding, updateDataValues, setAxisOrderFromValues } from '../utils/dataUtils';
...
export default function DataEditorDrawer({ spec, setSpec, tableCols, setTableCols, tableRows, setTableRows, palette, setPalette, onClose, saveProject }: DataEditorDrawerProps) {
  ...
  function applyDataEditor() {
    try {
      ...
      next = setAxisOrderFromValues(next);
  ...
  // Live-apply edits (debounced) so 시각화가 즉시 반영되도록
  useEffect(() => {
    try {
      const t = setTimeout(() => {
        try {
          ...
          if (palette && palette.length) {
            const colorEnc = findColorEncoding(next);
            if (colorEnc) { colorEnc.scale = colorEnc.scale || {}; colorEnc.scale.range = palette.slice(); }
          }
          setSpec(next);
        } catch (err) { /* ignore during typing */ }
      }, 250);
      return () => clearTimeout(t);
    } catch {}
  }, [tableRows, tableCols, palette]);
