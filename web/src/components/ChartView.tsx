...
export default function ChartView({ spec, aspect, palette }: { spec: any | null, aspect?: number, palette?: string[] }) {
  ...
  useEffect(() => {
    ...
    // Reduce text label collisions for bar charts by hiding labels on very small bars
    try {
      const tryDeoverlapText = (obj: any) => {
        if (!obj) return;
        const layers = Array.isArray(obj.layer) ? obj.layer : null;
        if (!layers) return;
        // find quantitative field used for bar length
        let valueField: string | null = null;
        for (const L of layers) {
          const markType = typeof L.mark === 'string' ? L.mark : L.mark?.type;
          if (markType === 'bar') {
            const enc = L.encoding || {};
            const xf = enc?.x?.field; const yf = enc?.y?.field;
            const xt = (enc?.x?.type || '').toLowerCase();
            const yt = (enc?.y?.type || '').toLowerCase();
            if (xt === 'quantitative' && typeof xf === 'string') { valueField = xf; break; }
            if (yt === 'quantitative' && typeof yf === 'string') { valueField = yf; break; }
          }
        }
        ...
        for (const L of layers) {
          const markType = typeof L.mark === 'string' ? L.mark : L.mark?.type;
          if (markType === 'text') {
            L.mark = { ...(typeof L.mark === 'object' ? L.mark : {}), type: 'text', fontSize: 11, limit: 80 };
            L.encoding = L.encoding || {};
            // Hide labels where the absolute value is too small to avoid overlap around zero
            L.encoding.opacity = { condition: { test: `abs(datum['${valueField}']) > 24`, value: 1 }, value: 0 } as any;
            // Align left/right away from zero depending on sign
            (L.encoding as any).align = { condition: { test: `datum['${valueField}'] < 0`, value: 'right' }, value: 'left' };
            (L.encoding as any).dx = { condition: { test: `datum['${valueField}'] < 0`, value: -4 }, value: 4 } as any;
          }
        }
      };
      tryDeoverlapText(clone);
    } catch {}
